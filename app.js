      ...
      const timer =
        setTimeout(
          () => {
            ...
            reject(
              new Error(
                'API連線逾時，請確認第8步 Apps Script 已重新部署。'
              )
            );
          },
          API_TIMEOUT_MS
...
async function login() {
  ...
  try {
    ...
    $('staffLabel')
      .textContent =
      `${currentStaff.name}｜${currentStaff.personId}`;
    ...
    $('dutyDate')
      .textContent =
      currentStaff.dutyDate ||
      '—';

    $('dutyStart')
      .textContent =
      currentStaff.dutyStart ||
      '—';

    $('dutyEnd')
      .textContent =
      currentStaff.dutyEnd ||
      '—';
...
async function processQr(raw) {
  ...
  try {
    ...
    $('pointCode')
      .textContent =
      `${cp.checkpointId}｜${cp.qrCode}`;

    $('maxDistance')
      .textContent =
      `${cp.maxDistanceMeters} 公尺`;
